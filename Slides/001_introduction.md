...
Dans la prochaine slide vous copiez/collez le code du fichier d'exemple et vous faites un `Go Live` depuis votre `vscode` pour afficher le résultat.
...
## Modèle mental (très important)

En React:

- Un composant est une **fonction**.
- Le rendu, c'est **exécuter la fonction** pour produire une description de l'UI.
...
Donc:
- si `props` ou `state` changent → React relance le rendu → met à jour l'écran.

---

## Composant vs élément

- **Composant**: une fonction (`function Hello() { ... }`)
- **Élément React**: un objet qui décrit quoi afficher
  - produit par `React.createElement(...)` ou JSX (`<Hello />`)

Exemple:

```jsx
const el1 = React.createElement("h1", null, "Hello");
const el2 = <Hello />;
```
...
La forme courte est très utilisée :
...
## **Exercice 4 — Affichage de la liste d'étudiants suivante**
...
## **Exercice 5 — Calcul de moyenne des étudiants suivants**
...
- React: UI déclarative, **arbre de composants**
- `React.createElement` / JSX: deux façons de produire des **éléments React**
- Rendu: React compare (diff) et met à jour uniquement ce qui change
- JSX: expressions `{}`, conditions, listes + `key`
- Sécurité: `{}` échappe le contenu, attention à `dangerouslySetInnerHTML`

---

## Merci d'avoir écouté cette première partie
